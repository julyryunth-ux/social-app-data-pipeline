...
## summary
25개 원본 테이블(4GB+)을 Airflow·Docker 기반으로 정제·적재하는 ETL 파이프라인을 구축한 프로젝트입니다.
담당: RAW → DW 적재 파이프라인, 유저 활동 데이터마트, 운영 대시보드
처리 규모: 25개 테이블 / 4GB+ / 유저 약 67.7만 건
Stack: Python · MySQL · Airflow · Docker Compose · Railway · Tableau
